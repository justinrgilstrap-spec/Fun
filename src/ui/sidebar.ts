...
interface Stats {
  ...
  parks: number;
  /** Power 4 + Independent FBS schools with any VenueState set (campus/stadium/
   *  game all count), 0–69. The rest of FBS (Group of Five) and all of FCS/MLB
   *  deliberately have no equivalent totalizer — search/checklist/map-only. */
  fbs: number;
...
// ACC (17) + Big 12 (16) + Big Ten (18) + SEC (16) + Independent (Notre Dame, UConn = 2).
// Deliberately excludes the rest of FBS (Group of Five) and all of FCS — see
// countPowerFbs() in src/geo/datasets.ts.
const POWER_FBS_SCHOOLS = 69;
...
export function renderStats(el: HTMLElement, stats: Stats): void {
  ...
  const fbsRow = stats.fbs > 0 ? progressRow("Power 4 Stadiums", stats.fbs, POWER_FBS_SCHOOLS) : "";
  el.innerHTML = `
